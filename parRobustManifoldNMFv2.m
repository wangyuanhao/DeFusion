function [X_f, Z_f, E_f, convergence] = parRobustManifoldNMFv2(D, alpha, gamma, lowDim, KNeigh, fout)
%% Input:
%   D: a cell contains data for all views
%   initX??matirx, initial for common sample space, sample_num x low_dim
%   initZ: cell, initial for feature space in all views, low_dim x feature_num
%   initE??cell, initial for error in all views, sample_num x feature_num
%   L: Laplacian matrix for manifold
%% model
...
for rp = 1:nrep
    ...
    while abs(oldEval - newEval) > (startEval - newEval)*1e-2 && (run <= maxiter)
        ...
        for xinner =1:maxinner
            ...
            deltaX = 1/(norm(sum(cat(3, ZZ{:}),3),'fro') + norm(L, 'fro'));
